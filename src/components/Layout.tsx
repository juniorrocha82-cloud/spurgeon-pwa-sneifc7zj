...
export default function Layout() {
  ...
  const getPageTitle = () => {
    ...
    if (location.pathname === '/bible') return 'Bíblia Sagrada'
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col md:flex-row font-sans selection:bg-primary/30">
      ...
      <aside
        ...
        className="hidden md:flex flex-col w-64 fixed inset-y-0 left-0 bg-card border-r border-border z-50 shadow-elevation"
      >
        <div className="h-20 flex items-center px-6 border-b border-border/50">
          <BookOpen className="w-7 h-7 text-primary mr-3" aria-hidden="true" />
          <span className="font-serif text-2xl font-bold text-primary tracking-wide">Spurgeon</span>
        </div>
        <nav aria-label="Menu principal desktop" className="flex-1 py-8 px-4 space-y-3">
          {navItems.map((item) => {
            const isActive =
      ...
      <main
        ...
        className="flex-1 md:ml-64 pb-20 md:pb-0 min-h-screen relative flex flex-col focus:outline-none"
        tabIndex={-1}
      >
        {/* Header */}
        <header
      ...
      <nav
        ...
        className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-card/95 backdrop-blur border-t border-border flex items-center justify-around z-50 px-2 pb-safe"
      >
        {navItems.map((item) => {
          const isActive =
            location.pathname === item.path ||
