...
export default function HistoryPage() {
  ...
  return (
    <section
      className="flex-1 flex flex-col w-full animate-fade-in-up"
    ...
    >
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="relative w-full md:w-72">
          ...
          <Input
            ...
            className="pl-9 bg-card border-border/50 focus-visible:ring-primary focus-visible:outline-none"
            value={searchTerm}
      ...
      {recentSermons.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-card/30 border border-border/30 rounded-2xl border-dashed">
          ...
          <Button
            onClick={() => navigate('/')}
            className="btn-gold-glow text-primary-foreground font-medium px-8 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
            aria-label="Navegar para a página de gerar sermão"
          >
            Gerar Sermão
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8" role="list">
          {filteredSermons.map((sermon) => (
            <Card
              ...
              tabIndex={0}
              onKeyDown={(e) => {
                if (e.key === 'Enter') navigate(`/sermon/${sermon.id}`)
              }}
              ...
              aria-label={`Ver detalhes do sermão: ${sermon.title}`}
            >
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Button
                    ...
                    className="h-8 w-8 text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-destructive transition-opacity focus:opacity-100 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
                    onClick={(e) => {
                      e.stopPropagation()
              ...
              <CardFooter className="pt-0 border-t border-border/30 mt-auto px-6 py-4 flex justify-between items-center text-xs text-muted-foreground">
                ...
                <div
                  ...
                  aria-label={`Duração de ${sermon.duration} minutos`}
                >
