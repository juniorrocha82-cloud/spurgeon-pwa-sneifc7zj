...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let userPage = 1;
    while (hasMoreUsers) {
      const { data: authData, error: authError } = await supabaseAdmin.auth.admin.listUsers({ page: userPage, perPage: 1000 });
    ...
    const mergedData = users.map(user => {
      ...
      if (sub.plan_id === 'pro' || sub.plan_id === 'enterprise' || (genLimit !== null && genLimit > 5)) {
      ...
      if (sub.plan_id !== 'enterprise' && user.id !== ADMIN_ID) {
        const userLogs = logs.filter(log => 
          log.user_id === user.id && log.resource_type === 'sermon' && new Date(log.created_at) >= startDate
        );
