...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!messages || !Array.isArray(messages)) {
      return new Response(JSON.stringify({ error: 'Parâmetro "messages" é obrigatório e deve ser um array.' }), {
    ...
    for (const provider of providers) {
      ...
      console.log(`[ROUTE] [${provider.provider_name}] Tentando ler a chave de API do Supabase Secrets (${envKeyName}) ou do banco de dados...`);
      let apiKey = Deno.env.get(envKeyName) || provider.api_key;

      if (!apiKey) {
        console.error(`[ROUTE] [${provider.provider_name}] ERRO: Chave de API não encontrada (${envKeyName}).`);
      ...
      try {
        ...
        if (providerName === 'gemini') {
          ...
          let baseUrl = provider.endpoint || `https://generativelanguage.googleapis.com/v1beta/models/${targetModel}:generateContent`;
          ...
          const fetchUrl = baseUrl.includes('?') ? `${baseUrl}&key=${apiKey}` : `${baseUrl}?key=${apiKey}`;
          ...
          const targetModel = model?.includes('llama') || model?.includes('mixtral') ? model : 'llama3-70b-8192';
          const baseUrl = provider.endpoint || 'https://api.groq.com/openai/v1/chat/completions';
          
          console.log(`[ROUTE] [${provider.provider_name}] Chamando ${baseUrl} (Modelo: ${targetModel})`);
          ...
          if (baseUrl.includes('/generate')) {
            const promptStr = messages.map((m: any) => `${m.role === 'system' ? 'System' : m.role === 'assistant' ? 'Assistant' : 'User'}: ${m.content}`).join('\n\n') + '\n\nAssistant:';
            ...
            for (const msg of messages) {
              ...
              else chatHistory.push({ role: msg.role === 'assistant' ? 'CHATBOT' : 'USER', message: msg.content });
            }
        ...
        const duration = Date.now() - startTime;
        console.error(`[ROUTE] Falha no provedor ${provider.provider_name} após ${duration}ms:`, err.message);
        logs.push({ provider: provider.provider_name, status: 'error', error: err.message, durationMs: duration });
      }
    }
