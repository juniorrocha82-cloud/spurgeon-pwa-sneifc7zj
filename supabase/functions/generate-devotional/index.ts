...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const themes = ['Graça', 'Fé', 'Esperança', 'Amor', 'Perdão', 'Obediência', 'Sabedoria', 'Paciência', 'Humildade', 'Justiça', 'Paz', 'Alegria', 'Santidade', 'Misericórdia'];
    const perspectives = ['pessoal', 'comunitária', 'histórica', 'profética'];
    const structures = ['meditativa', 'analítica', 'prática', 'contemplativa'];
    const verseTypes = ['Antigo Testamento', 'Novo Testamento', 'Salmos', 'Provérbios', 'Evangelhos', 'Epístolas Paulinas', 'Profetas Menores'];
    const depths = ['iniciante (acessível e inspirador)', 'intermediário (reflexivo e contextual)', 'avançado (profundo e teológico)'];
